...
def tag_yt_song(song_path: str) -> Song:
    yt_id = get_yt_id(song_path)
    if not yt_id:
        raise SongNotFoundError()
    song = VocaDB.search_by_yt_id(yt_id)
...
def tag_dir(song_dir: str, target_dir: str):
    for f in listdir(song_dir):
        if f.endswith(".mp3"):
            song_path = path.join(song_dir, f)
            try:
                song = tag_yt_song(song_path)
...
def main():
    if len(argv) == 3:
        ...
        print(
            f"usage: ./{argv[0]} <directory with downloaded mp3s> <target dir>"
        )
